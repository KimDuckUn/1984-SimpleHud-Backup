...
local bank = ""
local postals = {}
local leoActive = false
local fireActive = false
local emsActive = false
...
if config.enablePriorityStatus then
    TriggerEvent("chat:addSuggestion", "/priostart", "Start a priority.")
    TriggerEvent("chat:addSuggestion", "/cooldown", "Stop an active priority and sets timer.")
    TriggerEvent("chat:addSuggestion", "/resetpcd", "Start a cooldown on priorities with a custom time.", {
...
RegisterCommand("leo", function(source, args, rawCommand)
    if args[1] == "active" then
        leoActive = not leodActive
        local statusMessage = leoActive and "LEO: Active" or "LEO: Inactive"
        TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, statusMessage)
    else
        TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, "Usage: /leo active")
    end
end, false)
-- Los Santos Sheriff Command --

RegisterCommand("fire", function(source, args, rawCommand)
    if args[1] == "active" then
        fireActive = not fireActive
        local statusMessage = fireActive and "FIRE: Active" or "FIRE: Inactive"
        TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, statusMessage)
    else
        TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, "Usage: /fire active")
    end
end, false)
-- Fire Department --
RegisterCommand("ems", function(source, args, rawCommand)
    if args[1] == "active" then
        emsActive = not emsActive
        local statusMessage = emsActive and "EMS: Active" or "EMS: Inactive"
        TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, statusMessage)
    else
        TriggerEvent("chatMessage", "SYSTEM", {255, 0, 0}, "Usage: /ems active")
    end
end, false)

-- Command List
TriggerEvent("chat:addSuggestion", "/leo", "Toggle LEO status", {{name = "active", help = "Toggle LEO active status"}})
TriggerEvent("chat:addSuggestion", "/fire", "Toggle fire status", {{name = "active", help = "Toggle fire active status"}})
TriggerEvent("chat:addSuggestion", "/ems", "Toggle EMS status", {{name = "active", help = "Toggle EMS active status"}})
    ...
    while true do
        ...
        if not hidden then
            if config.enableAopStatus then
             ...
             local leoColor = leoActive and {r = 0, g = 255, b = 0, a = 255} or {r = 255, g = 0, b = 0, a = 255}
             local leoText = leoActive and "~g~LEO" or "~r~LEO"
             text(leoText, 0.168, 0.845, 0.40, 4, leoColor)

            -- Los Santos Sheriff Display
             local fireColor = fireActive and {r = 0, g = 255, b = 0, a = 255} or {r = 255, g = 0, b = 0, a = 255}
             local fireText = fireActive and "~g~FIRE" or "~r~FIRE"
             text(fireText, 0.182, 0.845, 0.40, 4, fireColor)
            -- Fire Department Display --

            local emsColor = emsActive and {r = 0, g = 255, b = 0, a = 255} or {r = 255, g = 0, b = 0, a = 255}
            local emsText = emsActive and "~g~EMS" or "~r~EMS"
            text(emsText, 0.198, 0.845, 0.40, 4, emsColor)        
        ...
        if vehicle ~= 0 and vehClass ~= 13 and driver then
            text(tostring(math.ceil(GetEntitySpeed(vehicle) * speedCalc)), 0.190, 0.818, 0.5, 4, {r = 0, g = 0, b = 0, a = 255})
            text(speedText, 0.210, 0.823, 0.45, 4)
